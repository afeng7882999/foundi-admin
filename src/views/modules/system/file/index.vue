  ...
  <div ref="moduleRoot" :style="pageMinHeight" class="page-file fd-page">
      ...
      <el-form ref="queryForm" :inline="true" :model="query" size="medium" @keyup.enter="queryList()">
        <transition name="expand" @enter="expandEnter" @after-enter="expandAfterEnter" @before-leave="expandBeforeLeave">
          <div v-show="queryFormShow" class="fd-page__query">
            ...
            <el-form-item label="文件上传的OSS配置键值" prop="oss">
              <fd-tree-select v-model="query.oss" :data-list="configList" :select-params="{ placeholder: 'OSS配置' }" :tree-fields="{ idField: 'configKey', labelField: 'configKey' }"></fd-tree-select>
            </el-form-item>
            <el-form-item label="文件类型" prop="typeDict">
              <el-select v-model="query.typeDict" size="medium">
                <el-option v-for="item in dicts.sysFileType" :key="item.itemKey" :label="item.itemValue" :value="item.itemKey"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item>
    ...
    <div class="fd-page__table border">
      <el-table v-loading="loading" :data="data" row-key="id" @selection-change="onSelectionChange">
        ...
        <el-table-column :show-overflow-tooltip="true" align="left" header-align="left" label="OSS配置" prop="oss" width="200"></el-table-column>
        ...
        <el-table-column :show-overflow-tooltip="true" align="left" header-align="left" label="创建时间" prop="createAt" width="160"></el-table-column>
        ...
        <el-table-column align="center" fixed="right" header-align="center" label="操作" width="160">
          <template #default="scope">
            <el-tooltip :show-after="500" content="复制当前文件连接" placement="top">
              <el-button v-show="hasAuth('system:file:list')" class="fd-tb-act" plain size="mini" type="primary" @click="copyUrl(scope.row.url, $event)">
                <fd-icon icon="link-one"></fd-icon>
                复制链接
              </el-button>
            </el-tooltip>
            <el-tooltip :show-after="500" content="删除" placement="top">
              <el-button v-show="hasAuth('system:file:delete')" class="fd-tb-act tb-act-delete" plain size="mini" type="danger" @click="del(scope.row, scope.row.k)">
