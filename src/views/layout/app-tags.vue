<template>
  <div class="app-tags">
    ...
    <fd-scroll-panel ref="scrollPanel" scroll-item-class-name="tags-view-item">
      <div class="tags-view-wrapper">
        <router-link v-for="tag in visitedViews" :key="tag.fullPath" :class="isActive(tag) ? 'active' : ''" :to="{ path: tag.path, name: tag.name, query: tag.query, params: tag.params, fullPath: tag.fullPath }" class="tags-view-item" tag="span" @contextmenu.prevent.stop="openMenu(tag, $event)">
          <span class="tags-view-title">{{ tag.title }}</span>
          <span v-if="!isAffix(tag)" class="tags-view-close" @click.prevent="closeSelectedTag(tag)">
            <fd-icon class="close-icon" icon="close-small"></fd-icon>
          </span>
          <span v-else class="tags-view-not-close"></span>
        </router-link>
      </div>
    </fd-scroll-panel>
    <div class="tags-view-ops">
    ...
    <fd-contextmenu ref="contextMenu" class="app-tags-context-menu">
      <fd-contextmenu-item icon="refresh" label="刷新" @click="refreshSelectedTag(selectedTag)"></fd-contextmenu-item>
      <fd-contextmenu-item v-if="closable" icon="close" label="关闭" @click="closeSelectedTag(selectedTag)"></fd-contextmenu-item>
      <fd-contextmenu-item label="关闭其他" icon="blank" @click="closeOthersTags"></fd-contextmenu-item>
      <fd-contextmenu-item label="关闭所有" icon="blank" @click="closeAllTags(selectedTag)"></fd-contextmenu-item>
    </fd-contextmenu>
  </div>
</template>
...
const DEFAULT_ROUTE = {
  ...
  fullPath: '/dashboard',
  matched: [],
  meta: { id: '300', title: '管理主页', icon: 'home', isDynamic: true, affix: true }
}
